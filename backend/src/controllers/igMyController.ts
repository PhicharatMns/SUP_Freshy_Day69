  ...
  try {
    ...
    if (imageFile && imageFile.size > 0) {
      if (!imageFile.type.startsWith("image/")) {
        return c.json(
          { success: false, message: "กรุณาอัปโหลดไฟล์รูปภาพที่ถูกต้อง" },
          400,
        );
      }
      ...
      try {
        ...
        return c.json(
          { success: false, message: "ไม่สามารถอัปโหลดรูปภาพได้" },
          500,
        );
      }
    }
    ...
    return c.json(
      { success: false, message: "เกิดข้อผิดพลาดภายในระบบหลังบ้าน" },
      500,
    );
  }
};
...
export const deleteIg = async (c: Context) => {
  try {
